<template>
	<div class="show-box"  :class="{disabled: disabled, active: isShowList}" :style="style_Container">
		<!-- 输入框，仅在可输入模式下使用 -->
		<input  :style="myFontA"
			v-if="showInput"  class="input"  placeholder-style=""
			type="text" v-model.trim="selectText"  :placeholder="placeholder"
			@focus.stop="onFocus" @blur.stop="onBlur" @input.stop="onInput" @confirm.stop="$emit('confirm', $event)"
		>
		<!-- 显示框 -->
		<div v-else class="input" :class="{placeholder: selectText === placeholder}" @click="onUpperClick" >{{selectText}}</div>

		<!-- 右侧的小三角图标 -->
		<span v-show="isCanInput"
			@click.stop="onUpperClick" 
		...
		:style="'top:' + listTop__ + 'px;'" v-show="isShowList">
			...
			>
				<div 
					class="item" @click.stop="onClickItem(index, item.value)"
